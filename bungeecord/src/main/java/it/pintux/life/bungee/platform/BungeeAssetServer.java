...
public class BungeeAssetServer implements PlatformAssetServer {
    ...
    public BungeeAssetServer(ProxyServer proxy, File dataFolder, int port) {
        this.proxy = proxy;
    ...
    public void start() {
        try {
            ...
            httpServer.start();
            String host = proxy.getServers().values().stream().findFirst().map(s -> s.getAddress().getHostString()).orElse("127.0.0.1");
    ...
    @Override
    public boolean isAvailable() { return httpServer != null; }
